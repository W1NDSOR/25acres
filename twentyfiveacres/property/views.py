...
def propertyList(request):
    ...
    if selected_budget:
        ...
        if price_range:
            properties = properties.filter(price__gte=price_range[0], price__lte=price_range[1])
    ...
    if selected_location_area:
        ...
        if area_range:
            properties = properties.filter(area__gte=area_range[0], area__lte=area_range[1])
    ...
    if selected_availability_date:
        ...
        if time_range:
            ...
            properties = properties.filter(availabilityDate__gte=time_range[0], availabilityDate__lte=time_range[1])
...
def generatePropertyHashIdentifier(title, description, price, bedrooms, bathrooms, area, status, location, availableDate):
    """
...
def addProperty(request):
    ...
    if request.method == "POST":
        ...
        field_names = ["title", "description", "price", "bedrooms", "bathrooms", "area", "status", "location", "available_date"]
        field_values = {}
        ...
        if (title and description and price and bedrooms and bathrooms and area and status and location and availableDate):
            latitude, longitude = geocode_location(location)
            locationObject = Location.objects.create(latitude=latitude, longitude=longitude,)
            locationObject.save()
            property = Property.objects.create(title=title, description=description, price=price, bedrooms=bedrooms, bathrooms=bathrooms, area=area, status=status, location=locationObject, owner=User.objects.get(username=request.user.username), availabilityDate=availableDate, propertyHashIdentifier=generatePropertyHashIdentifier(title, description, price, bedrooms, bathrooms, area, status, location, availableDate), currentBid=0, bidder=None,)
            property.save()
...
@login_required
@require_POST
def addBid(request, property_title):
