...
def handleContract(request, propertyId):
    ...
    if contract is None and property.owner == user:
        ...
        seller_contract_hash = str(sellerContract.contractHashIdentifier)
        print("contract_hash: ", seller_contract_hash)

        # Doesn't work
        signature = sign_with_portal_private_key(private_portal_key, seller_contract_hash)
        print("signature: ", signature)
        encrypted_signature = encrypt_with_user_sha(user_sha, signature)
        print("encrypted_signature: ", encrypted_signature)
        return HttpResponseRedirect("/user/profile")
    ...
    if (
        ...
        buyer_contract_hash = buyerContract.contractHashIdentifier
        signature = sign_with_portal_private_key(private_portal_key, buyer_contract_hash)
        encrypted_signature = encrypt_with_user_sha(user_sha, signature)
        ...
        send_mail(
            "Welcome to 25acres",
            f"Your Contract hash is {buyer_contract_hash} and encrypted signature is {encrypted_signature}",
            "settings.EMAIL_HOST_USER",
            [user.email],
            fail_silently=False,
        )
